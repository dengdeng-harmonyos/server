...
9. **时区处理**
   - **数据库时区**: PostgreSQL配置为UTC时区
   - **API时间戳**: 所有timestamp字段为UTC时间戳（毫秒）
   - **ISO 8601时间**: 所有时间字符串均使用ISO 8601格式的UTC时间（以Z结尾）
   - **客户端处理**: 客户端收到的timestamp已是UTC时间戳，可直接使用Date对象处理本地时间显示
   - **注意**: 服务端在提取时间戳时使用 `AT TIME ZONE 'UTC'` 确保统一
